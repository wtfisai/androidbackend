                ...
                html += `
                    <div class="list-group-item">
                        ...
                        ${iface.addresses.map(addr => 
                            `<div><small class="text-muted">${addr}</small></div>`
                        ).join('')}
                    </div>
                `;
            }
...
async function loadProcesses() {
    ...
    if (data && data.processes) {
        ...
        const html = data.processes.slice(0, 100).map(proc => `
            <tr>
                <td>${proc.pid}</td>
                <td>${proc.user}</td>
...
async function loadPackages() {
    ...
    if (data && data.packages) {
        ...
        const html = data.packages.map(pkg => 
            `<div class="badge bg-secondary m-1">${pkg}</div>`
        ).join('');
        container.innerHTML = html;
    } else {
...
async function loadStorageInfo() {
    ...
    if (data && data.storage) {
        const html = data.storage.map(fs => `
            <div class="card mb-2">
                <div class="card-body">
                    <h6>${fs.filesystem}</h6>
